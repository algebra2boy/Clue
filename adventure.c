...
struct Room *go(char *direction, struct Room **board, struct Avatar avatar){
    if (strcmp(direction, "north") == 0){
        if ((avatar.current_room->North) == NULL){ // check if that room exists or not
            printf("room does not exist, try again\n");
        }
        else{ // the room exists then investigate
        ...
        if ((avatar.current_room->South) == NULL){// check if that room exists or not
            printf("room does not exist, try again\n");
        }
        ...
        if ((avatar.current_room->West) == NULL){ // check if that room exists or not
            printf("room does not exist, try again\n");
        }
        else{ // the room exists
        ...
        if ((avatar.current_room->East) == NULL){ // check if that room exists or not
            printf("room does not exist, try again\n");
        }
        else{ // the room exists
...
void look(struct Avatar avatar){
    ...
    if (checkRoomExist(avatar.current_room->North) == 0){ // North
    ...
    if (checkRoomExist(avatar.current_room->South) == 0){ // South
    ...
    if (checkRoomExist(avatar.current_room->West) == 0){ // West
    ...
    if (checkRoomExist(avatar.current_room->East) == 0){ // East
...
int itemMatch(struct Avatar avatar, struct item *item_answer){ 
    ...
    struct item *current_item = avatar.inventory;
        while(current_item != NULL){ // traverse the linkedlist
            if(strcmp(current_item->item_name, item_answer->item_name) == 0){ // Found it 
                return !match; 
            }
...
int clue(char * character_name, struct Room **board, struct Avatar avatar, struct Room *room_answer, char *character_answer, struct item *list_item_answer){
    ...
    if(strcmp(avatar.current_room->name, room_answer->name) == 0){ // same room name
        printf("Room Match\n");
        room_match = 1;
    }
...
struct Room **removeCharacterFromBoard(struct Room **board, char * character_name){
    for(int i =0;i<3;i++){ // go through all rows
        for(int j = 0;j<3;j++){ // go through all columns
            for(int found = 0; found < 6;found++){ // traverse the character list
...
struct Avatar addCharacterToAvatarRoom(struct Avatar avatar, char *character_name){
    for(int found = 0;found < 6;found++){
        if(avatar.current_room->character[found] == NULL){ // find empty slot, then assign value 
...
int main(){
    ...
    while (clue_Attempt < 10){ // losing state: if the player cannot hit all 3 matches after she made her 10th clue command.
        ...
        if (strcmp(first, "help") == 0){
            ...
            if(checkDirectionValid(strcpy(temp, second)) ==0){ // invalid direction
            ...
            if(checkItemNameValid(second,list_item)){ // retype an ITEM name if the item name is not valid
            ...
            if(checkCharacterNameValid(second, character_name)){
                ...
                if(state){ // winning state: hit all three states in one clue
                    printf("CONGRATULATIONS, YOU HAVE SOLVED THE MYSTERY!\n");
                    break;  // break out immediately when the avatar has figure out  
                }
                printf("\n");
            }
        }else{ // allow an avatar to retype a command if the command is not one of the above
            printf("No such command exists, try again\n");
        }
        printf("\n");
        clue_Attempt ++; // incremented the the number of attemp that is up to 10
    }
    if(clue_Attempt == 10){ // losing state: if the player cannot hit all 3 matches after he/she made his/her 10th clue command.
